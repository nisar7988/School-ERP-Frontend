...
export function DashboardPage() {
  ...
  const { data: paymentsData, isLoading: isLoadingPayments } = usePayments({ limit: 5 })
  ...
  const recentActivities = paymentsData?.data?.map((p: any) => {
    ...
    return {
      ...
      status: "PAID",
      time: p.paidAt ? formatDistanceToNow(new Date(p.paidAt), { addSuffix: true }) : 'Recently',
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-8">
          ...
          <div className="h-[400px]">
            <AttendanceTrends data={trendData} isLoading={isLoadingAttendance} />
          </div>
        </div>
        ...
        <div className="col-span-12">
           <RecentActivity activities={recentActivities} isLoading={isLoadingPayments} />
        </div>
      </div>
    </div>
  )
